...
@section('content')
    <div class="container mx-auto px-4">
        ...
        <div class="popular-games text-sm grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 lg:grid-cols-6 gap-12 border-b border-gray-800 pb-16">
            ...
            @foreach($gameCovers as $game)
            <div class="game mt-8">
                <div class="relative inline-block">
                    <a href="#">
                        <img src="{{ Str::replaceFirst('thumb', 'screenshot_huge', $game['cover']['url'] ?? '') }}" alt="game cover" class="hover:opacity-75 transition ease-in-out duration-150">
                ...
                <a href="" class="block text-block font-semibold text-base mt-8 hover:text-gray-400">{{ $game['game']['name'] }}</a>
                <div class="text-gray-400 mt-1">
{{--                    @if(array_key_exists('abbreviation', $platform)) @endif--}}
                    {{ $game['game']['platforms'][0]['abbreviation'] ??  '' }}
                </div>
            </div>
            @endforeach
            ...
            <div class="recently-reviewed w-full lg:w-3/4 mr-0 lg:mr-32">
                ...
                <div class="text blue-500 uppercase tracking-wide font-semibold">
                    <div class="recently-reviewed-container space-y-12 mt-8">
                        @foreach($recentReviews as $game)
                        <div class="game bg-gray-800 shadow-md rounded-lg flex px-6 py-6">
                            <div class="relative flex-none">
                                <a href="#">
                                    <img src="{{ Str::replaceFirst('thumb', 'screenshot_huge', $game['cover']['url'] ?? '') }}" alt="game cover" class="w-48 hover:opacity-75 transition ease-in-out duration-150">
                                </a>
                                <div class="absolute bottom-0 right-0 w-16 h-16 bg-gray-400 rounded-full" style="right:-20px; bottom: -20px">
                                    <div class="font semibold text-sm flex items-center justify-center h-full">
                                        80%
